...
const DateSelect = ({dateTime, id}) => {
    ...
    const navigate = useNavigate(); //when date is choosen and then click on the book now button then to redirect it on the seat select page

    const [selected, setSelected] = useState(null);

    /* to handle the book now button */
    const onBookHandler = () => {
        if(!selected){ //if we dont select the date and click on the book now button
